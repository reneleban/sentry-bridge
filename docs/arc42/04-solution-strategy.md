...
SentryBridge is a thin translation layer: it speaks the PrusaLink HTTP API on one side and the Obico Moonraker WebSocket protocol on the other. No protocol is extended or modified — the bridge adapts between them.
...
| Agent identity     | Moonraker agent protocol (`moonraker_obico` v2.1.0) | Moonraker identity enables `http.tunnelv2` for the Obico file browser tab; OctoPrint identity does not support the tunnel protocol (ADR-002) |
