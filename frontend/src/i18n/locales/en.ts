export default {
  wizard: {
    title: "Setup Wizard",
    steps: {
      prusalink: "PrusaLink",
      camera: "Camera",
      obico: "Obico",
      done: "Done",
    },
    prusalink: {
      heading: "Connect PrusaLink",
      description: "Enter the URL and credentials for your PrusaLink printer.",
      ip: "Printer IP Address",
      ip_placeholder: "192.168.1.x",
      username: "Username",
      username_placeholder: "maker",
      password: "Password",
      test: "Test Connection",
      testing: "Testing…",
      success: "Connection successful",
      error: "Connection failed",
      next: "Next",
    },
    camera: {
      heading: "Camera",
      description:
        "Your printer's RTSP stream will be used for live monitoring.",
      rtsp_url: "RTSP URL",
      rtsp_placeholder: "rtsp://192.168.1.x/live",
      test: "Test Stream",
      testing: "Testing…",
      success: "Stream reachable",
      error: "Stream not reachable",
      preview: "Preview",
      next: "Next",
      back: "Back",
    },
    obico: {
      heading: "Connect Obico",
      description:
        "Enter your Obico server URL and the verification code shown in the Obico setup wizard.",
      server_url: "Obico Server URL",
      server_placeholder: "http://192.168.1.x:3334",
      pairing_code: "Verification Code",
      code_hint:
        "Open the Obico setup wizard — it will show you a 6-digit code. Enter it here.",
      verify: "Verify",
      success: "Paired successfully",
      already_paired: "Already paired — no new pairing required",
      continue: "Continue",
      relink: "Re-link",
      error: "Pairing failed",
      back: "Back",
    },
    done: {
      heading: "Setup Complete",
      description:
        "Your printer is connected to Obico. You can now monitor and control it from the dashboard.",
      go_dashboard: "Go to Dashboard",
    },
  },
  dashboard: {
    title: "Dashboard",
    status: {
      prusalink: "PrusaLink",
      obico: "Obico",
      camera: "Camera",
    },
    health: {
      heading: "Service Health",
      overall: {
        healthy: "HEALTHY",
        degraded: "DEGRADED",
        recovering: "RECOVERING",
        down: "DOWN",
      },
      status: {
        connected: "Connected",
        connecting: "Connecting...",
        disconnected: "Disconnected",
        degraded: "Degraded",
      },
      reconnect: "Reconnect",
      last_checked: "Last checked: {{n}}s",
      last_checked_minutes: "Last checked: {{n}}m",
      stale: "Stale {{n}}s",
      advanced_details: "Advanced Details",
      loading: "Loading...",
    },
    printer: {
      heading: "Printer",
      state: "State",
      nozzle: "Nozzle",
      bed: "Bed",
      job: "Job",
      progress: "Progress",
      time_printing: "Elapsed",
      time_remaining: "Remaining",
      no_job: "No active job",
    },
    controls: {
      pause: "Pause",
      resume: "Resume",
      cancel: "Cancel",
      confirm_cancel: "Really cancel the print?",
    },
    camera: {
      heading: "Camera",
      no_frame: "No frame available",
    },
    fileBrowser: {
      title: "G-code Files",
      upload: "Upload",
      print: "Print",
      delete: "Delete",
      confirm_delete: "Delete {{name}}?",
      no_files: "No files on printer",
      loading: "Loading files…",
      error_load: "Failed to load files",
      error_upload: "Upload failed",
      error_print: "Failed to start print",
      error_delete: "Failed to delete file",
      col_name: "Name",
      col_size: "Size",
      col_date: "Date",
      file_count: "{{count}} file(s)",
      search_placeholder: "Search by filename…",
      no_search_results: "No file matches your search",
      overwrite_title: "File already exists",
      overwrite_confirm:
        "A file with this name already exists. Upload as «{{suggestedName}}» instead?",
      overwrite_yes: "Yes, rename & upload",
    },
    config: {
      heading: "Configuration",
      name: "Printer Name",
      name_placeholder: "My Printer",
      name_hint: "Leave empty to use the hostname from PrusaLink",
      prusalink_url: "PrusaLink URL",
      username: "Username",
      password: "Password",
      rtsp_url: "RTSP URL",
      frame_interval: "Frame Interval (s)",
      obico_url: "Obico Server URL",
      bridge_url: "Bridge URL (empty = auto-detect)",
      save: "Save",
      saving: "Saving…",
      saved: "Saved",
      error: "Failed to save",
    },
  },
  common: {
    language: "Language",
    theme: "Theme",
    light: "Light",
    dark: "Dark",
    system: "System",
  },
};
